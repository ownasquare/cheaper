...
## 1. Publish the CLI (this is the `npx cheaper` path)

The CLI bundles the gateway and plugin assets, so `npx cheaper install` is
self-contained.

```bash
cd cli
npm publish --access public      # name: "cheaper" (claim it, or scope as @ownasquare/cheaperapp)
```

After publish, `npx cheaper install --all` works anywhere Node 16+ is present.
...
`cheaper` package and builds the native installers (`.dmg` / `.exe` / `.deb` /
...
or to any static host. Wire the Download dropdown links and `npx cheaper` to your
...
- **`cheaper`** (`.github/workflows/publish-cli.yml`) — on a `v*` tag, verifies the
  tag matches `cli/package.json` and publishes `cheaper` to npm.
...
for `cheaper`. Signing / notarization secrets live in `cheaper-desktop`.

## Suggested release checklist

1. `cd gateway && python -m venv .venv && .venv/bin/pip install -r requirements.txt && .venv/bin/python -m pytest` (19 tests green).
2. Verify `npx cheaper install --all` on a clean machine (installs skill + agents +
   hook + gateway). Then `npx cheaper install plugin` and confirm
